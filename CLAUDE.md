...
### The bounded-wake invariant

Any blocking external call made **while holding the per-agent lock**, **on a bounded thread pool**, or **awaited by `drain()`** MUST carry a finite wall-clock bound, set at the **composition root** ([`app.py`](src/basecradle_router/app.py)) — never left to a library default of "wait forever." The wake is the load-bearing case: it is all three at once (it runs inside [`AgentLocks.guard`](src/basecradle_router/concurrency.py), on the shared `to_thread` pool, and is what `drain()` awaits), so an unbounded `claude` pins one agent's lock and a worker thread forever, saturates the small pool, and stalls wakes for the **whole fleet** — and hangs a deploy/reboot drain until systemd SIGKILLs the control-group mid-task (basecradle-router#135, finding C1). The bound is belt-and-suspenders: the wake-runner's OS-level `timeout(1)` reaps the whole `runuser→bash→claude` tree, and the router's own `subprocess.run` timeout is the Python-level backstop. When you add any new blocking boundary subject to those three conditions, give it a configured, finite bound the same way.
...
This is **not** a published package — the router daemon **runs on the home server**, not installed as a library. No PyPI/release pipeline; the daemon reaches the box by the **NOC's** deploy (the capital, in the interim), **never by this agent** — see "Building vs. Deploying" below.
...
- **The deploy mechanism is authored here, run by the deployer.** `deploy/` holds the version-controlled deploy config and the self-verifying deploy loop ([`deploy/deploy.sh`](deploy/deploy.sh), [`deploy/smoke-test.sh`](deploy/smoke-test.sh), [`deploy/drift-check.sh`](deploy/drift-check.sh), the systemd units) — this agent **authors and maintains** them as *code*, because the box's deploy config is version-controlled in the box's own repo (`constitution.md` → New-Server Provisioning). But the **NOC/capital runs them**, never the router-AI. `deploy/deploy.sh` carries a hard guard that refuses to run without an explicit deployer acknowledgment (`DEPLOYER=noc`), so a reflexive self-deploy cannot happen again.
