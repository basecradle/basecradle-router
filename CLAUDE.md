...
- **One interface, one path:** `python -m basecradle_router {claims,selftest freeze,evidence}`, wrapped on the box by `deploy/bin/router-admin` (privilege drop + env). The CLI is strictly **read-only** — a probe run under the wrong identity must never take the evidence file's ownership away from the daemon. Exit codes, TTLs, and the on-box invocation: [`deploy/README.md`](deploy/README.md) → "Proving what the router claims."
